				...
				<view class="wrap-item content">
					...
					<view class="step-wrap u-p-40">
						<el-steps simple :active="stepIndex" finish-status="success">
							...
							<el-step title="绑定新手机" icon="el-icon-lock"></el-step>
					...
					<view class="form-wrap">
						...
						<template v-else-if="stepIndex == 1">
							<el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-width="100px" label-position="left">
								<el-form-item label="新手机号码" prop="phone">
									<el-input v-model="ruleForm2.phone" clearable></el-input>
								</el-form-item>
								<el-form-item label="手机验证码" prop="code2">
									<el-row>
										<el-col :span="16">
						...
						<template v-else>
							<el-result icon="success" title="成功绑定手机" >
								<template slot="extra">
	...
	export default {
		data() {
			return {
				...
				rules: {
					code: { required: true, len: 6, message: '请输入6位验证码', trigger: ['blur', 'change']}
				},
				rules2: {
					phone: [{ required: true, message: '请输入手机号码', trigger: ['blur', 'change']}],
					code2: { required: true, len: 6, message: '请输入6位验证码', trigger: ['blur', 'change']}
				}
			}
		},
		onLoad() {
